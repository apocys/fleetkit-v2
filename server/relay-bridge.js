(function() {
    ...
    function buildTaskGraphSvg(missions) {
        ...
        svg += '<defs><linearGradient id="skTaskEdge" x1="0" y1="0" x2="1" y2="0"><stop offset="0%" stop-color="rgba(0,122,255,0.3)"/><stop offset="100%" stop-color="rgba(88,86,214,0.6)"/></linearGradient></defs>';
        ...
        missions.forEach(function(m, idx) {
            ...
            svg += '<text x="' + x + '" y="' + (centerY + 44) + '" text-anchor="middle" font-size="11" fill="var(--text-primary)" style="font-family: system-ui, -apple-system, BlinkMacSystemFont, sans-serif;">' + label + '</text>';
            svg += '</g>';
        });
    ...
    // ── Missions Tab (F12) — Task Graph + Kanban ──
